...
export class ChangelogService extends Loggable {
  ...
  private async generateChangelogContent(options: {
    ...
    // Override writerOpts to use correct template paths
    if (presetConfig?.writerOpts) {
      presetConfig.writerOpts.mainTemplate = await fs.readFile(path.join(templatesPath, 'template.hbs'), 'utf-8');
      presetConfig.writerOpts.headerPartial = await fs.readFile(path.join(templatesPath, 'header.hbs'), 'utf-8');
      presetConfig.writerOpts.commitPartial = await fs.readFile(path.join(templatesPath, 'commit.hbs'), 'utf-8');
      presetConfig.writerOpts.footerPartial = await fs.readFile(path.join(templatesPath, 'footer.hbs'), 'utf-8');
      this.log.debug('Templates loaded successfully');
    }
