...
export class ChangelogService extends Loggable {
  ...
  private async generateChangelogContent(options: {
    ...
    // Create writer options manually with templates as strings
    // This avoids calling the preset config which tries to load files
    const presetConfig = {
      writerOpts: {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        transform: (commit: any, context: any) => {
          // Discard invalid commits
          if (!commit || typeof commit.type !== 'string') {
            return null;
          }

          // Create a new commit object to avoid immutability issues
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          const newCommit: any = { ...commit };

          // Map commit types to friendly titles
          ...
          newCommit.type = typeMap[commit.type] || commit.type;

          // Ensure commit has required arrays and map notes
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          newCommit.notes = (commit.notes || []).map((note: any) => {
            return {
              ...note,
          ...
          // Generate commit links if repository context available
          if (context && context.host) {
            ...
            if (context.repository && context.owner) {
              newCommit.repository = context.repository;
              newCommit.owner = context.owner;
            }
          }

          // Filter references that have an issue number
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          newCommit.references = (commit.references || []).filter((ref: any) => {
            return ref.issue && !isNaN(parseInt(ref.issue, 10));
          });

          return newCommit;
        },
        groupBy: 'type',
        commitGroupsSort: (a: { title: string }, b: { title: string }) => a.title.localeCompare(b.title),
        commitsSort: ['scope', 'subject'],
        noteGroupsSort: (a: { title: string }, b: { title: string }) => a.title.localeCompare(b.title),
        mainTemplate,
        headerPartial,
        commitPartial,
        footerPartial,
    ...
    this.log.debug('Writer options created with inline templates');
    ...
    this.log.debug(
      { parsedCommits, context, hasWriterOpts: !!presetConfig.writerOpts },
      'About to generate changelog string',
    );
    try {
      // Pass writerOpts directly, not the preset config wrapper
      const changelog = await writeChangelogString(parsedCommits, context, presetConfig.writerOpts);
      this.log.debug({ length: changelog.length, preview: changelog.substring(0, 200) }, 'Changelog generated');

      // If empty, log the inputs for debugging
      if (!changelog || changelog.trim().length === 0) {
        this.log.warn(
          {
            parsedCommitsCount: parsedCommits.length,
            parsedCommitsPreview: parsedCommits.slice(0, 2),
            context,
            writerOptsKeys: Object.keys(presetConfig.writerOpts),
          },
